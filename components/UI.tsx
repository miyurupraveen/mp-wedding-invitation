...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const variants = {
    primary: "bg-wedding-gold text-white hover:bg-yellow-600 focus:ring-wedding-gold shadow-md",
    secondary: "bg-white text-wedding-charcoal border border-wedding-stone/30 hover:bg-wedding-cream focus:ring-wedding-stone",
    danger: "bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 focus:ring-red-500",
    ghost: "bg-transparent text-wedding-charcoal hover:bg-wedding-cream/50"
  };
...
export const Input: React.FC<InputProps> = ({ label, error, className = '', ...props }) => {
  return (
    <div className="w-full">
      {label && <label className="block text-sm font-medium text-wedding-charcoal/80 mb-1 font-serif">{label}</label>}
      <input
        className={`w-full px-3 py-2 bg-white border rounded-lg focus:outline-none focus:ring-2 focus:ring-wedding-gold/20 focus:border-wedding-gold transition-colors ${error ? 'border-red-300' : 'border-wedding-stone/30'} ${className}`}
        {...props}
      />
...
export const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <div className={`bg-white rounded-xl shadow-lg border border-wedding-stone/10 p-6 ${className}`}>
    {children}
  </div>
);
